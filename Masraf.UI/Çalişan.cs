...
namespace Masraf.UI
{
    public partial class Çalişan : Form
    {
        ...
        {
            ndata.SaveChanges();
            dataGridView1.DataSource = ndata.Masraflar.ToList();
        }
        public Çalişan()
        {
            InitializeComponent();
        ...
        {
            ndata.SaveChanges();
            dataGridView1.DataSource = mRepo.SelecAll();
        }
        ...
        private void Çalişan_Load(object sender, EventArgs e)
        {
        ...
        private void comboBox1_SelectionChangeCommitted(object sender, EventArgs e)
        {
            if (comboBox1.SelectionLength > 0)
            {
                ...
                dataGridView1.DataSource = mRepo.selectMasrafID(masrafismi).Where(f=>f.YetkiRol==3).Select(f => new
                {
                    f.Baslik,
                    f.Tutar,
                    f.Acıklama,
